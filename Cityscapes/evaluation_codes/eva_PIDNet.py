...
def main():
    ...
    parser.add_argument('--eval_scale', type=float, default=0.5, help='scale factor for evaluation resolution (e.g., 0.5 for half size)')
    args = parser.parse_args()

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"Using device: {device}")
    print(f"Evaluation scale: {args.eval_scale}")
    ...
    # 2. 画像とGTのペアを作成 (Cityscapes 構造に対してより強力なマッチング)
    img_list = []
    
    print(f"Searching for images in: {os.path.abspath(args.img_dir)}")
    print(f"Searching for GTs in: {os.path.abspath(args.gt_dir)}")
    ...
    for root, _, files in os.walk(args.img_dir):
        for f in files:
            if f.endswith(('.png', '.jpg', '.jpeg')):
                all_imgs.append((f, os.path.join(root, f)))

    if not all_imgs:
        print(f"No images found in {args.img_dir}. Please check if the path is correct.")
        return
    else:
        print(f"Found {len(all_imgs)} images. Attempting to match with GTs...")

    # GTファイルをすべて取得（一度スキャンしてインデックス化）
    gt_map = {} # キー: ファイル名(拡張子なしの一部), 値: フルパス
    for root, _, files in os.walk(args.gt_dir):
        for f in files:
            if f.endswith('.png'):
                # ID部分（例: frankfurt_000000_000294）を抽出
                # Cityscapesの標準的な命名規則: city_seq_frame_suffix.png
                parts = f.split('_')
                if len(parts) >= 3:
                    city_id = "_".join(parts[:3])
                    # 優先度の高いサフィックスを保持
                    if city_id not in gt_map or '_labelTrainIds' in f:
                        gt_map[city_id] = os.path.join(root, f)

    # マッチング
    for f_name, img_path in all_imgs:
        parts = f_name.split('_')
        if len(parts) >= 3:
            city_id = "_".join(parts[:3])
            if city_id in gt_map:
                img_list.append((img_path, gt_map[city_id]))
            # else:
            #     print(f"Debug: No GT found for {f_name} (ID: {city_id})")

    if not img_list:
        print("\nMatching failed. Some debug info:")
        print(f"Example image found: {all_imgs[0][0]}")
        if gt_map:
            print(f"Example GT ID indexed: {list(gt_map.keys())[0]} -> {list(gt_map.values())[0]}")
        else:
            print("No GT files with 3-part ID format (e.g., frankfurt_000000_000294) found in GT directory.")
        return
    if not img_list:
    ...
    h_eval = int(h_orig * args.eval_scale)
    w_eval = int(w_orig * args.eval_scale)
    # Ensure divisible by 32 for better compatibility
    h_eval = (h_eval // 32) * 32
    w_eval = (w_eval // 32) * 32
    ...
    if get_model_complexity_info:
        try:
            with torch.cuda.device(0) if device.type == 'cuda' else torch.cpu():
                macs, params = get_model_complexity_info(model, (3, h_eval, w_eval), as_strings=True,
    ...
    with torch.no_grad():
        for i, (img_path, gt_path) in enumerate(img_list):
            ...
            if args.eval_scale != 1.0:
                h_target, w_target = h_eval, w_eval
                img_resized = cv2.resize(img_rgb, (w_target, h_target), interpolation=cv2.INTER_LINEAR)
            else:
                img_resized = img_rgb
            ...
            if i == 0:
                ...
                res_img.save('first_prediction_resized.png')
                print("First prediction saved as 'first_prediction_resized.png'")
    ...
    output_lines.append(f"Original Resolution: {w_orig}x{h_orig}")
    output_lines.append(f"Evaluation Resolution: {w_eval}x{h_eval} (Scale: {args.eval_scale})")
    ...
    report_filename = f"evaluation_resized_results_{timestamp}.txt"
